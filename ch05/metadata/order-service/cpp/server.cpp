...
public:
    ...
    Status addOrder(ServerContext* context, const ecommerce::Order* request, StringValue* response) override 
    {
        ...
        if (metadata.size() < 2)
        {
            return Status(StatusCode::DATA_LOSS, "UnaryEcho: failed to get metadata");
        }

        for (auto iter = metadata.begin(); iter != metadata.end(); ++iter) {
           if (iter->first == "timestamp")
           {
            std::cout << "==> timestamp from metadata: " << iter->second <<std::endl;
           }
        }

        context->AddInitialMetadata("location", request->destination());
        context->AddInitialMetadata("timestamp", getCurrentTime());
...
private:
    ...
    std::string getCurrentTime()
    {
        ...
        std::tm* tm_info = std::localtime(&currentTime_t);
