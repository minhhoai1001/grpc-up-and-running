...
public:
    ...
    Status processOrders(ServerContext* context, grpc::ServerReaderWriter< CombinedShipment, StringValue>* stream) override {
        ...
        while (stream->Read(&orderID)) 
        {
            ...
            {
                std::cout <<" Context Cacelled for this stream: -> context canceled" << std::endl;
			    std::cout << "Stopped processing any more order of this stream! \n"; 
                return Status(grpc::StatusCode::CANCELLED, "Cacelled Stream");
            }
